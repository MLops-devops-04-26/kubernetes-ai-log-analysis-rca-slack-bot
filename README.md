...
```bash
cd aiops-bot
docker build -t gvsiva2008/aiops-bot:latest .
docker push gvsiva2008/aiops-bot:latest
# Verify push
...
```bash
cd ../load-generator
docker build -t gvsiva2008/load-generator:latest .
docker push gvsiva2008/load-generator:latest
# Verify push
...
```bash
cd ../microservices
docker build -t gvsiva2008/sample-service:latest .
docker push gvsiva2008/sample-service:latest
# Verify push
...
./update-deployments.sh gvsiva2008
```
...
Defines the local three-node Kind cluster used to run the labs.
...
```bash
kubectl apply -f microservices/deployment.yaml -n microservices
```
...
4. **Slack Bot Issues**
   ...
   ```
kubectl port-forward -n monitoring deployment/prometheus-grafana 32000:3000
kubectl port-forward -n monitoring pod/prometheus-prometheus-kube-prometheus-prometheus-0 9090:9090
kubectl port-forward -n microservices deployment/service1 31022:8000
kubectl port-forward -n monitoring deployment/prometheus-kube-prometheus-alertmanager 9093:9093
## Maintenance
